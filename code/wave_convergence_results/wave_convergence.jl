...
for (ieps, epsilon) in enumerate([1.0 0.4 0.05])
    for basis in [GaussLegendre, LobattoLegendre]
        ...
        for deg in [0,1, 2, 3]
            ...
            writedlm(joinpath(@__DIR__,"./$(basissstr)_eps=$(epsilon)_order=$(deg+1)_data.txt"), data)
            plot!(stepsizes, errors, label = "order $(deg+1)")
            plot!(title = L"$\epsilon =  %$(epsilon)$, basis = %$(basissstr)")
        end
        savefig(joinpath(@__DIR__,"./wave_$(basissstr)_eps=$(epsilon)_convergence.pdf"))

        for deg in [0,1, 2, 3]
            plot!(save_stepsizes, (save_stepsizes.+0.0).^(-(deg+1)), label = "reforder $(deg+1)")
        end
        savefig(joinpath(@__DIR__,"./wave_$(basissstr)_eps=$(epsilon)_convergence(incl_reflines).pdf"))
    end
end
...
#=
steps_and_error = readdlm(joinpath(@__DIR__,"./wave_convergence_results/Gauss_eps=0.05_order=2_data.txt"),'\t', Float64,'\n')
