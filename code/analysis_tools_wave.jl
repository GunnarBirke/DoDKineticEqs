...
function test_convergence_wave_background(basis, deg ; TMM = SSPRK3, exprange = [3,8], CFL = 0.1, a=1, epsilon = 0.4)
    stepsizes = 2 .^range(exprange[1],exprange[2])
    errors = zeros(length(stepsizes))
    for (istep, stepsize) in enumerate(stepsizes)
        problem = setup_problem_eq("sin", 0, 1, stepsize, Tmax = 5, a = a, CFL = CFL, bcs = "periodic", epsilon = epsilon);
        RHS_mat, problem = DGsemidiscretization_DoD_telegraph(problem, deg, basis, "Upwind",  do_stabilize = false, eq_type = "wave", fluxtype = "full");
